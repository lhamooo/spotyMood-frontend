...
export default function SpotifySearcher() {
  ...
  function handleSubmit() {
    setBackground(mood);
    const apiUrl = `https://spotymood-backend.netlify.app/api/playlists?limit=${limit}&q=${mood}`;
  ...
  function renderPlaylist() {
    ...
    for (const playlist of response) {
      playlists.push(
        <Playlist key={playlist.internUrl} playlist={playlist} background={background} />
      );
    }
    ...
    return <Container className="small-cards">{cardRows}</Container>;
  }
  ...
  return (
    <div className="body">
      ...
      <Form className="d-flex justify-content-center align-items-center flex-column">
        ...
        <br />
        <Button
          className={`${background}-button-submit`}
          variant="light"
          value="Submit"
      ...
      {background === 'home' ? (
        ...
        <Button
          className={`${background}-scroll-to-top-button`}
          variant="light"
          onClick={scrollToTop}
        >
